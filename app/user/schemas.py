...
class UserSchema(Schema):
    username = fields.Str(required=True, validate=validate.Length(min=8, max=16))
    first_name = fields.Str(required=True, validate=validate.Length(min=8, max=16))
    last_name = fields.Str(required=True, validate=validate.Length(min=8, max=16))
    phone_number = fields.Str(required=True, validate=validate.Length(min=8, max=32))
    email = fields.Email(required=True)
    password_hash = fields.Str(required=True, validate=validate.Length(min=8, max=128))
